...
import { services, proofPoints, testimonials, clientLogos, techCategories, caseStudies } from "../data/content";
import { ctaConfig } from "../config/cta";
import { heroImages, sectionImages } from "../config/images";
import { ArrowRight, ShieldCheck, FileText, Settings, BarChart3, Check } from "lucide-react";
...
const approachTabs: TabItem[] = [
  { id: "ownership", label: "Ownership", title: "Operational accountability, not headcount", description: "We contract for outcomes — working software, infrastructure uptime, quality metrics — not hours billed or seats filled. You review performance with our service lead, not manage our team's daily tasks.", linkLabel: "About AStechnix", linkHref: "/company/about-us" },
  { id: "governance", label: "Governance", title: "SLA-governed from day one", description: "Defined service levels, monthly reviews, and documented action items replace informal check-ins and person-dependent coordination. You always have visibility into how the service is performing.", linkLabel: "SLA Governance", linkHref: "/delivery-model/sla-governance-reporting" },
  { id: "transparency", label: "Transparency", title: "Full visibility into performance", description: "Operational dashboards and monthly reports give you complete transparency into service performance. No selective metrics, no hidden issues — just factual reporting with improvement plans.", linkLabel: "Explore services", linkHref: "/services/software-engineering-outsourcing" },
  { id: "integrity", label: "Integrity", title: "Factual integrity over marketing", description: "We don't invent proof points. If we don't have a certification, we say so. If case studies aren't verified, we don't publish them. What you see is what you get — no embellishment.", linkLabel: "Contact us", linkHref: "/contact" },
];
  ...
  return (
    <>
      ...
      <HeroSection
        ...
        title={<>We own your technology operations, <span className="text-brand-accent">end to end.</span></>}
      ...
      >
        ...
        <Button to={ctaConfig.contextual.exploreServices.path} variant="outline-white" size="lg">{ctaConfig.contextual.exploreServices.label}</Button>
      </HeroSection>

      <ClientLogos logos={clientLogos} />

      <StatsBar points={proofPoints} />

      <section className="section-padding bg-white">
        <div className="container-content">
          <SectionHeader
            eyebrow="What We Do"
            title={<>Five managed service lines, <span className="emphasis">one delivery framework.</span></>}
      ...
      <ImageContentSection
        ...
        title={<>Managed outsourcing vs. <span className="emphasis">staff augmentation</span></>}
        variant="tinted"
      >
        <p className="text-body">The distinction is accountability. Staff augmentation gives you people to manage. Managed outsourcing gives you a service outcome with governance.</p>
        <div className="space-y-3 pt-2">
          {["We own the service outcome — not just the headcount", "SLA framework is included, not an extra", "Monthly governance reports, not informal check-ins", "Single point of accountability — AStechnix"].map((point) => (
        ...
        <div className="pt-2"><Button to="/delivery-model/sla-governance-reporting" variant="secondary">See the full comparison</Button></div>
      </ImageContentSection>
      ...
      <ImageContentSection
        ...
        title={<>What differentiates <span className="emphasis">our delivery model</span></>}
        reverse
      >
      ...
      <TabbedContent
        eyebrow="How We Work"
        heading={<>How do we do it? The <span className="emphasis">AStechnix way</span></>}
        tabs={approachTabs}
      />
      ...
      <ImageContentSection
        ...
        eyebrow="Security"
        title={<>Operational security built into <span className="emphasis">the delivery model</span></>}
        variant="tinted"
      >
        <p className="text-body">Security is not a bolt-on. It's embedded in how we structure engagements, manage access, and handle data throughout the entire delivery lifecycle.</p>
        <div className="space-y-3 pt-2">
          {["Least-privilege access to your systems", "100% IP ownership transfer — contractually guaranteed", "Enforceable NDAs signed by every team member", "Defined data handling protocols aligned to your requirements"].map((point) => (
            <div key={point} className="flex items-start gap-3">
              <Check className="mt-0.5 h-5 w-5 shrink-0 text-brand-accent" />
              <p className="text-body-sm">{point}</p>
            </div>
        ...
        <div className="pt-2"><Button to="/delivery-model/security-compliance-ip-protection" variant="secondary">Security & IP Framework</Button></div>
      </ImageContentSection>
      ...
      <section className="section-padding bg-white border-y border-ink/10">
        <div className="container-content">
          <SectionHeader
            eyebrow="Technology"
            title={<>Capability areas we <span className="emphasis">work across</span></>}
      ...
      <section className="section-padding bg-white border-y border-ink/10">
        <div className="container-content max-w-3xl">
          <SectionHeader eyebrow="FAQ" title="Questions we hear from executives" align="center" className="mb-8" />
